...
# this package
from theape import BaseClass, ConfigurationError
...
class IperfConstants(object):
    ...
    value_expression = re.compile(STRING_START + SPACES + DIGITS + DOT + ZERO_OR_ONE +
                                  OPTIONAL_DIGITS + '[aAbBkKmMgG]' + ZERO_OR_ONE + SPACES + STRING_END)
    byte_expression = re.compile(STRING_START + SPACES + DIGITS + '[ABKMG]' + ZERO_OR_ONE + SPACES + STRING_END)
    window_expression = re.compile(STRING_START + SPACES + DIGITS + '[bBkKmM]' + ZERO_OR_ONE + SPACES + STRING_END)
    bandwidth_expression = re.compile(STRING_START + SPACES + DIGITS + '[kKmM]' + SPACES + STRING_END)
    num_expression = re.compile(STRING_START + SPACES + DIGITS + '[bBkKmMgG]' + ZERO_OR_ONE + SPACES + STRING_END)
...
class IperfBaseSettings(BaseClass):
    ...
    def set_number(self, attribute, value, lower_bound=None, caster=int):
        ...
        :raises: CameraobscuraError if not a non-negative number
        """
        if type(value )in IperfConstants.number_types:
            ...
            try:
                ...
                raise CameraobscuraError("{0} must be number or castable to a {1}, not {2}".format(attribute.lstrip('_'), caster,
                                                                                                value))
        if (lower_bound is not None and
            getattr(self, attribute) < lower_bound and
            value is not None):
            raise CameraobscuraError("{0} must be greater than {1}, not {2}".format(attribute.lstrip("_"),
                                                                                 lower_bound,
                                                                                 value))
    ...
    def set_bytes(self, attribute, value):
        ...
        :raises: CameraobscuraError if value is a string and doesn't match the form 'n[KM]'
        """
        setattr(self, attribute, value)
...
class IperfGeneralSettings(IperfBaseSettings):
    ...
    def format(self, new_format):
        ...
        :raise: CameraobscuraError if format is unknown
        """
        self._format = new_format
        if new_format is not None and new_format not in IperfGeneralConstants.valid_formats:
            raise CameraobscuraError("Invalid Iperf Format : {f} needs to be one of {v}".format(f=new_format,
    ...
    def interval(self, interval):
        ...
        :raise: CameraobscuraError if not castable to non-negative number
        """
    ...
    def len(self, new_len):
        ...
        :raise: Cameraobscura error of not a non-negative number or string of form n[KM]
        """
    ...
    def output(self, filename):
        ...
        :raise: ConfigurationError if filename has white-space
        """
        self._output = filename
        if not IperfConstants.no_whitespace_expression.search(str(filename)):
            raise ConfigurationError("filename must be string of non-whitespace characters, not {0}".format(filename))
    ...
    def port(self, new_port):
        ...
        :raise: CameraobscuraError if new-port is not integer
        """
    ...
    def reportexclude(self, exclusions):
        ...
        if exclusions is not None:
            if (type(exclusions) is not StringType):
                raise CameraobscuraError("reportexclude options must be a string, not '{0}'".format(exclusions))

            if not IperfConstants.reportexclude_expression.search(exclusions):
                raise CameraobscuraError("reportexclude options can only be one of [CDMSV] (case-insensitive), not '{0}'".format(exclusions))
    ...
    def reportstyle(self, c_or_C):
        ...
        if c_or_C is not None:
            if type(c_or_C) is not StringType:
                raise CameraobscuraError("reportstyle must be 'c' or 'C', not {0}".format(c_or_C))
...
class IperfCompositeBase(IperfBaseSettings):
    ...
    def set(self, attribute, value):
        ...
        :raise: CameraobscuraError if this class or IperfGeneralSettings doesn't have attribute
        """
        if hasattr(self.general_settings, attribute):
            ...
            raise CameraobscuraError("Unknown Attribute: {0}".format(attribute))
        return
    ...
    def get(self, attribute):
        ...
        :raise: CameraobscuraError if the attribute is unknown
        """
...
class IperfClientSettings(IperfCompositeBase):
    ...
    def prefix(self):
        ...
        :raise: CameraobscuraError if self._server is None
        """
        if self._prefix is None:
            try:
                ...
                self.logger.error(error)
                raise CameraobscuraError("server hostname must be set before use -- currently {0}".format(self.server))
        return self._prefix
    ...
    def listenport(self, new_port):
        ...
        :raise: CameraobscuraError if not a number or less than 1024
        """

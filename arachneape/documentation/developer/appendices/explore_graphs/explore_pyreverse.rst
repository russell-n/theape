...
That still is not a particularly exciting example, at least not visually. It is interesting that it picked up the two variables that hold the two aggregated classes within the Hortator but not the class names. Here is the example given on the `pyreverse <http://www.logilab.org/blogentry/6883>`_ blog for showing all dependencies but stripping out the detail to make it more comprehensible:
...
In this case we specified the class so it will only document TheHortator, even if there are other classes in the file. It creates a file named ``TheHortator.png``

.. image:: TheHortator.png

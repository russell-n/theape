...
The `fetch_config` method used to copy an example configuration file to the current working directory (thus its name). Since each plugin can provide its own sample, I decided that they should output them to standard out. Thus the user could do something like this::

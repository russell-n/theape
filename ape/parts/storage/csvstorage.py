...
10 11 12"""
    ...
    rows = (dict(zip(headers, row.split())) for row in raw_rows)
    storage = ape.parts.storage.filestorage.FileStorage(path='tests')
    with storage.open('test.csv') as testfile:
        pass
    #with open('tests/test.csv', 'wb') as testfile:
